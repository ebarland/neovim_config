...
vim.api.nvim_create_autocmd("FileType", {
	pattern = "cmake",
	callback = function()
		-- Prefer a portable `cmake-format` in PATH; allow overriding via $CMAKE_FORMAT.
		local function find_cmake_format()
			local env = vim.env.CMAKE_FORMAT
			if env and #env > 0 then return env end
			for _, c in ipairs({ "cmake-format", "cmake-format.exe" }) do
				if vim.fn.executable(c) == 1 then return c end
			end
			return nil
		end

		local cmake_format = find_cmake_format()
		if not cmake_format then
			-- Don't override <leader>fd if the formatter isn't available.
			return
		end

		vim.keymap.set("n", "<leader>fd", function()
			local formatted = vim.fn.system({ cmake_format, vim.fn.expand("%:p") })
		...
		end, { buffer = true, desc = "Format CMake file (cmake-format)" })
	end,
})
